...
const FileCard: FunctionComponent<{ file: ElectronFile }> = ({ file }) => {
  ...
  const [buttonInner, setButtonInner] = useState(<div>Copy Text</div>)
    ...
    <div className="fileCardParent">
      <div className="fileCardTop">
        <div className="fileAttributes">
          ...
          <div>Original File Size: {humanFileSize(file.size)}</div>
          <div>Converted Characters: {base64.length.toLocaleString('en-US')}</div>
        </div>
        <img src={base64} width={100} />
        <button
          className="copyButton"
          onClick={(): Promise<void> =>
            navigator.clipboard.writeText(base64).then((): void =>
              setButtonInner(
